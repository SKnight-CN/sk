...
user_email_address = 'ruiheng_charles@163.com'
# 邮箱的授权码，注意：不是登录密码
user_password = 'FZUDVEZSFTONVSOG'
...
def parse_email_server(email_server):
    ...
    for i in range(1,index+1):
        ...
        f=open('F:/newtime.txt', 'w+', encoding='utf-8')
        newest_time = f.read()
        if (max_mail_time_str > newest_time):
             continue
        f.write(max_mail_time_str)
        f.close()
...
def parser_content(msg, indent):
    ...
    for part in msg.walk():
        ...
        att_file = open('./emails' + filename, 'wb')  # 在指定目录下创建文件，注意二进制文件需要用wb模式打开
