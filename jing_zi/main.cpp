...
int cal(int player, int cur_state,int *ca, int *cb, int alpha, int beta) {
    int remain, t, ans;
    int ex_cur_state;
    t=*cb;
    ...
    while (remain) {
        int move = (remain & (-remain));
        ex_cur_state=cur_state;
        ex_cur_state=next_state(ex_cur_state, move,  cb);
        ans=alpha_beta(1, ex_cur_state, alpha, beta, *ca, *cb);
        if (ans==-1)
            return move;
    }
}
...

{
    ...
    int cur_state = 0, ex_cur_state,  new_state; // 当前局面
    ...
    while (cur_state!=end_state) {
        ...
        scanf("%d %d", &x, &y);
        ta=ca;
        cur_state = next_state(cur_state, 1<<edge[x][y],  &ca);
        ...
        b:flag =0;
        move=cal(0,cur_state,&ca, &cb, alpha, beta);
        tb=cb;
        cur_state=next_state(cur_state, 1<<move, &cb);
        for (int i=0;i<11;i++) {
            for (int j = 0; j < 11; j++)
                if (edge[i][j] == move) {
                    printf("SK chose:\"%d, %d\"\n", i, j);
